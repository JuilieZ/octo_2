...
To run this script, run:
    ...
    /home/server/octo/checkpoint_rm75_04
"""
...
# sys.path.append("/home/server/octo/examples/envs")
        ...
        while len(images) < 400:
            ...
            if done or trunc:
                if done:
                    print("Oops")
                break
        print("Length of imgs",len(images))
        print(f"Episode return: {episode_return}")

        # log rollout video to wandb -- subsample temporally 2x for faster logging
        wandb.log(
            {"rollout_video": wandb.Video(np.array(images).transpose(0, 3, 1, 2)[::3])}
        )

...
class PDEEPosController(PDJointPosController):
    ...
    def compute_target_pose(self, prev_ee_pose_at_base, action):
        ...
        if self.config.use_delta:
            ...
            # delta_pose.raw_pose[]
            # print(">>>>>>>>>>>>>>>>action",action)
            # print("111111111111111111111   delta_pose",delta_pose)
        ...
        # target_pose.raw_pose = np.hstack([target_pose.raw_pose[:3],torch.tensor([0,1,0,0]).cuda()])
        # target_pose.raw_pose = torch.cat([torch.tensor(target_pose.raw_pose[:,:3]).cuda(), torch.tensor([0,1,0,0]).repeat([target_pose.raw_pose.shape[0],1]).cuda()], dim=1)
        # print("22222222222222 prev_ee_pose_at_base",prev_ee_pose_at_base)
        # print(">>>>>>>>>>>>target_pose",target_pose.raw_pose.shape)
        return target_pose
    ...
    def set_action(self, action: Array):
        ...
        # print("####post_action:",action)
        # print("prev_ee_pose_at_base",self.ee_pose_at_base)
        ...
        # print("####_target_pose:",self._target_pose)
        # breakpoint()

...
class TableSceneBuilder(SceneBuilder):
    ...
    def initialize(self, env_idx: torch.Tensor):
        ...
        if self.env.robot_uids == "panda":
            ...
            qpos = np.array(
                [
                    ...
                    # # pullcube tool  didn't make sense
                    # -22.072 * np.pi/180,
                    # -2.208 * np.pi/180,
                    # 21.139 * np.pi/180,
                    # 96.965 * np.pi/180,
            ...
            # qpos[:, -3] = 0
            self.env.agent.reset(qpos)
            self.env.agent.robot.set_pose(sapien.Pose([-0.21, -0.0, 0.008]))
        elif self.env.robot_uids == "rm75_wristcam":
            # fmt: off
            qpos = np.array([ 0,0,0,np.pi*1/2,0,np.pi*1/2, -np.pi,
                                #   0,+np.pi*1/36
                    0.8,0.8,0.8,0.8,
                ]
            )
            # qpos = np.array([0,57.4*np.pi/180,91.6*np.pi/180,0,88.3*np.pi/180,-122.6*np.pi/180,0,0.8,0.8,0.8,0.8,])
            ...
            qpos[:, -4:] = 0.8
            self.env.agent.reset(qpos)
            self.env.agent.robot.set_pose(sapien.Pose([-0.210, 0, 0.00]))
        ...
        elif self.env.robot_uids == "rm75_stick":
            qpos = np.array(
                [
                    -29.595*np.pi/180,
                    -59.249*np.pi/180,
            ...
            self.env.agent.reset(qpos)
            self.env.agent.robot.set_pose(sapien.Pose([-0.210, 0, 0]))

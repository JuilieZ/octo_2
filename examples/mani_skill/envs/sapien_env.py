...
class BaseEnv(gym.Env):
    ...
    def __init__(
        ...
        self._control_mode = "pd_ee_delta_pose"
    ...
    def _load_lighting(self, options: dict):
        ...
        self.scene.add_directional_light([0, 0, -1], [1.2,1.2,1])
    
    def convert_obs(self, obs, im_size):
        # 假设 rgb_data 是一个 torch tensor
        rgb_data = obs['sensor_data']['hand_camera']['rgb']
        # 使用 torchvision.transforms 来调整大小
        self.transforms = T.Compose(
            [
                T.Resize((im_size, im_size), antialias=True),
            ]
        )

        image_obs = self.transforms(rgb_data.permute(0, 3, 1, 2) )  # 变为 uint8
        image_obs = image_obs.permute(0,2,3,1)
        image_obs = image_obs[0]
        # image_obs = (np.array(obs['sensor_data']['hand_camera']['rgb'][0]).resize([im_size,im_size,3]).astype(np.uint8))
        # add padding to proprio to match training
        proprio = obs['agent']['qpos'][0]
        # print("proprio>>>",proprio)
        # NOTE: assume image_1 is not available
        return {
            # "image_primary": image_obs,
            "image_wrist": image_obs,
    ...
    def reset(self, seed: Union[None, int, list[int]] = None, options: Union[None, dict] = None):
        ...
        info = self.get_info()
        raw_obs = self.get_obs(info)
        im_size = 128
        obs = self.convert_obs(raw_obs, im_size)
        # print(obs['proprio'])
        # breakpoint()
    ...
    def step(self, action: Union[None, np.ndarray, torch.Tensor, Dict]):
        ...
        if raw_obs is None:
            ...
            truncated = True
            obs = null_obs(im_size)  # obs with all zeros
        else:
            obs = self.convert_obs(raw_obs, im_size)
